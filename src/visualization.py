...
def plot_label_distribution(df: pd.DataFrame) -> None:
    ...
    counts = (
        df["label"].value_counts().reindex(LABEL_NAMES).fillna(0).astype(int)
    )
    total = counts.sum()
...
def plot_signals_by_category(df: pd.DataFrame) -> None:
    ...
    sns.boxplot(data=df, x="label", y="stars", hue="label", order=order,
                palette=LABEL_PALETTE, legend=False, **box_kws, ax=axes[0, 0])
    axes[0, 0].set_title("Stars"); axes[0, 0].set_xlabel(""); axes[0, 0].set_ylabel("z-score")
    axes[0, 0].tick_params(axis="x", rotation=30)

    sns.boxplot(data=df, x="label", y="contributors_count", hue="label", order=order,
                palette=LABEL_PALETTE, legend=False, **box_kws, ax=axes[0, 1])
    axes[0, 1].set_title("Contributors"); axes[0, 1].set_xlabel(""); axes[0, 1].set_ylabel("z-score")
    axes[0, 1].tick_params(axis="x", rotation=30)

    sns.boxplot(data=df, x="label", y="repo_age_days", hue="label", order=order,
                palette=LABEL_PALETTE, legend=False, **box_kws, ax=axes[1, 0])
    axes[1, 0].set_title("Repository Age"); axes[1, 0].set_xlabel("LLM label"); axes[1, 0].set_ylabel("z-score")
    axes[1, 0].tick_params(axis="x", rotation=30)

    ci_rate = df.groupby("label")["has_ci"].mean().reindex(order).fillna(0).mul(100)
    bar_colors = [LABEL_PALETTE[l] for l in ci_rate.index]
    axes[1, 1].bar(ci_rate.index, ci_rate.values, color=bar_colors, edgecolor="white", linewidth=0.6)
    for i, (_, val) in enumerate(ci_rate.items()):
        axes[1, 1].text(i, val + 1.5, f"{val:.0f}%", ha="center", fontsize=9)
    axes[1, 1].set_title("CI/CD Presence Rate"); axes[1, 1].set_xlabel("LLM label")
    axes[1, 1].set_ylabel("% of repos with CI"); axes[1, 1].set_ylim(0, 115)
...
def plot_confusion_matrix_bert() -> None:
    if not EVAL_JSON.exists():
        logger.warning("Skipping confusion_matrix_bert.png — %s not found.", EVAL_JSON)
        return
    with open(EVAL_JSON) as f:
        data = json.load(f)
    if "confusion_matrix" not in data:
        logger.warning("Skipping confusion_matrix_bert.png — 'confusion_matrix' key missing.")
        return
    ...
    sns.heatmap(cm_norm, annot=annot_norm, fmt="", cmap="Blues", vmin=0, vmax=1,
                xticklabels=labels, yticklabels=labels,
...
def plot_model_comparison() -> None:
    if not COMPARISON_CSV.exists():
        logger.warning("Skipping model_comparison.png — %s not found.", COMPARISON_CSV)
        return
    ...
    for _, row in overall.iterrows():
        ml = name_map.get(row["model"], row["model"])
        for m in metrics:
            records.append({"Model": ml, "Metric": m.capitalize(), "Score": float(row[m])})
    long_df = pd.DataFrame(records)

    fig, ax = plt.subplots(figsize=(10, 5))
    model_palette = {"Baseline (LR)": "#A8C5E2", "DistilBERT": "#2B6CB0"}
    sns.barplot(data=long_df, x="Metric", y="Score", hue="Model",
                palette=model_palette, edgecolor="white", linewidth=0.6, ax=ax)
    for container in ax.containers:
        ax.bar_label(container, fmt="%.3f", padding=3, fontsize=9)
    ax.set_ylim(0, 1.12)
    ax.set_title("Model Comparison — Baseline vs DistilBERT  (weighted averages, test set)",
                 fontweight="bold", pad=12)
